...
import { posts, users, comments as mockComments } from '../data/mock';
import { useAuth } from '../context/AuthContext';
import { Comment as CommentType, Post, ReportStatus } from '../types';
...
const PostDetail: React.FC = () => {
  ...
  const [postData, setPostData] = useState<Post | undefined>(post);
  ...
  return (
    <div className="bg-secondary shadow-xl rounded-lg p-4 md:p-8">
      <header className="border-b-2 border-accent pb-4 mb-6">
        <h1 className="text-4xl font-bold text-text-primary">{postData.title}</h1>
            ...
            return (
              <div key={comment.id} className="bg-accent p-4 rounded-lg">
                <div className="flex items-start">
                    {commentAuthor ? <Link to={`/profile/${commentAuthor.id}`}><img src={commentAuthor.avatarUrl} alt={commentAuthor.nickname} className="w-10 h-10 rounded-full mr-4" /></Link> : <div className="w-10 h-10 rounded-full mr-4 bg-gray-300"></div>}
